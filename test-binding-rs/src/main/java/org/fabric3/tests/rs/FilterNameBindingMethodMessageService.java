...
@Scope("COMPOSITE")
public class FilterNameBindingMethodMessageService {
    ...
    public Message retrieve(@PathParam("id") Long id) {
        try {
            ...
            if (!TestNameRequestFilter.SET) {
                throw new AssertionError("Name filter not applied");
            }
    ...
    @GET
    @Path("messagenofilter/{id}")
    public Message retrieveNoFilter(@PathParam("id") Long id) {
        try {
            if (!TestRequestFilter.SET) {
                throw new AssertionError("Global filter not set");
            }
            if (TestNameRequestFilter.SET) {
                throw new AssertionError("Name improperly not applied");
            }
            return new Message(id, "this is a test");
        } finally {
            TestRequestFilter.SET = false;
