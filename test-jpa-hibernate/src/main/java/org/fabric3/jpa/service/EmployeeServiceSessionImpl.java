...
public class EmployeeServiceSessionImpl implements EmployeeService {
    ...
    public void removeEmployee(Long id) {
        Employee employee = (Employee) employeeSession.load(Employee.class, id);
        employeeSession.delete(employee);
    }

    public ExEmployee findExEmployee(Long id) {
        return (ExEmployee) exEmployeeSession.get(ExEmployee.class, id);
    }

    public void removeExEmployee(Long id) {
        ExEmployee exEmployee = (ExEmployee) exEmployeeSession.get(ExEmployee.class, id);
        exEmployeeSession.delete(exEmployee);
    }

    @SuppressWarnings({"unchecked"})
    public List<Employee> searchWithCriteria(String name) {
        Criteria criteria = employeeSession.createCriteria(Employee.class);
